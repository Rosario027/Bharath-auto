        ...
        <section className="fsec">
          ...
          {series.map((s) => (
            <div className="series-row" key={s.id}>
              ...
              <span className={`badge ${s.docType === 'purchase-order' ? 'po-open' : `vt-${s.docType === 'credit-note' ? 'credit-note' : s.docType === 'debit-note' ? 'debit-note' : 'sales'}`}`} style={{ alignSelf: 'center' }}>
                {s.docType === 'credit-note' ? 'Credit Note' : s.docType === 'debit-note' ? 'Debit Note' : s.docType === 'purchase-order' ? 'Purchase Order' : 'Invoice'}
              </span>
